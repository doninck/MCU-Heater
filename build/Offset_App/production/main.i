...
uint8_t EUSART_Read_N(uint8_t * Rcv_Buff);
# 348 "./mcc_generated_files/eusart.h"
uint8_t EUSART_Read(void);
# 368 "./mcc_generated_files/eusart.h"
void EUSART_Write(uint8_t txData);
# 389 "./mcc_generated_files/eusart.h"
void EUSART_Transmit_ISR(void);
# 410 "./mcc_generated_files/eusart.h"
...
# 525 "./mcc_generated_files/eusart.h"
...
const char NVFlag __attribute__((address(0x2000 - 1))) = 0x55;
...
{
    NVMADRL = (uint8_t) (0x2000 -2) & 0x00FF;
    NVMADRH = (uint8_t) (((0x2000 -2) & 0xFF00) >> 8);
    NVMDATL = 0x00;
    NVMDATH = 0x00;
    NVMCON1 = 0x14;
    NVMCON2 = 0x55;
...
{
    NVMADRL = (uint8_t) (0x400 & 0x00FF);
    NVMADRH = (uint8_t) ((0x400 & 0xFF00) >> 8);
    NVMDATL = 0x00;
    NVMDATH = 0x00;
    NVMCON1 = 0x14;
    NVMCON2 = 0x55;
...
void init(void)
{
    ...
    (INTCONbits.GIE = 1);
    (INTCONbits.PEIE = 1);
        for(i=0;i<(1 +3 +sizeof(Default_PWM_Table));i++){
        Read_EEPROM[i] = DATAEE_ReadByte(0xF000 +i);
    }

    if(Read_EEPROM[0] == 0xFF){
        DATAEE_WriteByte(0xF000, 0xAA);
        for(i=0;i<sizeof(Default_PWM_Table);i++){
            DATAEE_WriteByte(0xF000 +1 +3 +i, Default_PWM_Table[i]);
        }
...
void UartResponse_ReadCmd(uint8_t type)
{
   ...
   switch (type) {
        ...
        case 0x0A:
            ...
            reg[0] = 0x40;
            I2C1_RBlk(0x5C, reg, 1, Fan_PWM_Value, 1);
            sendBuff[0] = 0x02;
            sendBuff[1] = 0x03;
            sendBuff[2] = cmd.Data_Type;
            sendBuff[3] = Fan_PWM_Value;
            printf("Fan_PWM_Value is %d \n", Fan_PWM_Value);
            break;
        case 0x0C:
            sendBuff[0] = 0x02;
            sendBuff[1] = 0x04;
        ...
        case 0x0D:
            sendBuff[0] = 0x02;
            sendBuff[1] = 2 + sizeof(Default_PWM_Table);
            sendBuff[2] = cmd.Data_Type;
            for(i=0;i<sizeof(Default_PWM_Table);i++){
                sendBuff[i+3] = DATAEE_ReadByte(0xF000 +1 +3 +i);
            }
            break;
        case 0x0E:
            sendBuff[0] = 0x02;
            sendBuff[1] = 0x05;
            sendBuff[2] = cmd.Data_Type;
            sendBuff[3] = PORTAbits.RA2;
            sendBuff[4] = PORTCbits.RC0;
            sendBuff[5] = PORTCbits.RC1;
            break;
        case 0x0F:
            sendBuff[0] = 0x02;
            sendBuff[1] = 0x04;
...
void UartResponse_WriteCmd(uint8_t type, uint8_t value)
{
    ...
    switch (type){
        case 0x03:
            CurrentPWM1 = value;
            PWM5_LoadDutyValue((CurrentPWM1*319)/100);
            sendBuff[0] = 0x04;
            sendBuff[1] = 0x03;
            sendBuff[2] = type;
            sendBuff[3] = CurrentPWM1;
        ...
        case 0x04:
            CurrentPWM2 = value;
            PWM6_LoadDutyValue((CurrentPWM2*319)/100);
            sendBuff[0] = 0x04;
            sendBuff[1] = 0x03;
            sendBuff[2] = type;
            sendBuff[3] = CurrentPWM2;
        ...
        case 0x0A:
            Fan_PWM_Value = value;
            printf("Fan_PWM_Value is %d value is %d \n", Fan_PWM_Value, value);
        ...
        case 0x0D:
            DATAEE_WriteByte(0xF000, 0xAA);
            for(i=0;i<sizeof(Default_PWM_Table);i++){
                DATAEE_WriteByte(0xF000 +1 +3 +i,cmd.Value[i]);
            }
            flg_reset = 1;
            break;
        default:
...
void Handler_Uart( void )
{
   ...
   if ((eusartRxCount==0) || (eusartRxCount>64)) return;
    ...
    rx_len = EUSART_Read_N((uint8_t*)rcv_Buff);
    ...
    if ( (cmd.Command_type==0x01)&&(rx_len == 3) ) {
        UartResponse_ReadCmd(cmd.Data_Type);
    } else if(cmd.Command_type == 0x03 && rx_len <= 0x0E) {
...
void Handler_I2c( void)
{
    ...
    I2C1_RBlk(0x40, Reg, 1,ReadBuffer, 2);
